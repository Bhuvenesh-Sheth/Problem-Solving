...
public class Node_at_distance_from_leafnode {
...
class Solution {
    ...
    void fun(Node root, ArrayList<Node> v, int k, HashSet<Node> ans) {
        if (root == null)
            return;

        if (root.left == null && root.right == null && v.size() >= k) {
            ans.add(v.get(v.size() - k));
            return;
        }

        v.add(root);
        fun(root.left, v, k, ans);
        fun(root.right, v, k, ans);
        v.remove(v.size() - 1);
    }
